...
def fix_pose_data(df_path):
    ...
    # Other papers do this, Im not a big fan of it, Smoothing the data is probably better
    # likelihood_threshold = 0.75
    # for column_group in df.columns[1::3]:  # Skipping the Image column
    ...
    tail_root_y = "TailRoot_y"
    body_center_y = "BodyCenter_y"
    if tail_root_y in df.columns and body_center_y in df.columns:
        df.loc[df[tail_root_y] < df[body_center_y], tail_root_y] = df[body_center_y]

        for k in range(1, len(df)):
            if df.loc[k, tail_root_y] < df.loc[k, body_center_y]:
                df.loc[k, tail_root_y] = df.loc[k - 1, tail_root_y]

    df.to_csv(df_path, index=False)
    return original_column_names, relative_column_names
